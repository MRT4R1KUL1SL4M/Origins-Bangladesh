        ...
        const MENU_STRUCTURE = [
            ...
            { title: "Product & Heritage", id: "group-product", icon: "ph-package", items: [{ id: "products", label: "Product Catalog", icon: "ph-stack" }, { id: "qc", label: "Quality Control", icon: "ph-clipboard-text" }, { id: "gi-verify", label: "GI Certification", icon: "ph-medal" }, { id: "heritage-atlas", label: "Heritage Atlas", icon: "ph-map-trifold" }] },
        ...
        let MOCK_ATLAS_DATA = Array.isArray((window.__ADMIN_DASHBOARD__ || {}).atlas) ? (window.__ADMIN_DASHBOARD__.atlas) : [];
        ...
        let FEATURED_QUEUE = [];
        let ATLAS_DISTRICTS = [];
        let ATLAS_MAP_POINTS = [];
        let ATLAS_PENDING_SELECTION = null;
        let ATLAS_SVG_READY = false;
        let ATLAS_SVG_DOTS = {};
        let DOC_VIEWER_STATE = { docs: [], activeId: null, zoom: 1 };
        ...
        function normalizeAtlasDistrictName(name) {
            return String(name || '')
                .replace(/&apos;/g, "'")
                .replace(/&amp;apos;/g, "'")
                .trim()
        ...
        function canonicalAtlasDistrictName(name) {
            const normalized = normalizeAtlasDistrictName(name);
            return ATLAS_NAME_ALIASES[normalized] || normalized;
        }

        function getAtlasDistrictRecordByName(name) {
            const canonical = canonicalAtlasDistrictName(name);
            return (ATLAS_DISTRICTS || []).find(d => canonicalAtlasDistrictName(d.name) === canonical) || null;
        }

        function getAtlasMapPointByName(name) {
            const canonical = canonicalAtlasDistrictName(name);
            const raw = (ATLAS_MAP_POINTS || []).find(p => canonicalAtlasDistrictName(p.zilla) === canonical) || null;
            const svgDot = ATLAS_SVG_DOTS[canonical] || null;
            const rawDot = raw && raw.dot && Number(raw.dot.x) >= 0 && Number(raw.dot.x) <= 100 && Number(raw.dot.y) >= 0 && Number(raw.dot.y) <= 100 ? raw.dot : null;
            if (!raw && !svgDot && !rawDot) return null;
            return {
                ...(raw || {}),
                dot: svgDot || rawDot || null,
                shop_link: (raw && raw.shop_link) || `/shop?district=${encodeURIComponent(name || '')}`
            };
        }

        function updateAtlasSvgActiveState(activeName) {
            const svg = document.querySelector('#atlas-map-svg-host svg');
            if (!svg) return;
            const activeCanonical = canonicalAtlasDistrictName(activeName);
            svg.querySelectorAll('path[id]').forEach(path => {
                const isActive = canonicalAtlasDistrictName(path.getAttribute('id')) === activeCanonical;
                path.classList.toggle('atlas-svg-active', isActive);
                path.style.fill = isActive ? '#C27803' : '#111111';
                path.style.opacity = isActive ? '1' : '0.96';
            });
        }

        async function ensureAtlasSvgLoaded() {
            const host = document.getElementById('atlas-map-svg-host');
            if (!host) return null;
            if (ATLAS_SVG_READY && host.querySelector('svg')) return host.querySelector('svg');
            const res = await fetch('/static/assets/maps/geoBoundaries-BGD-ADM2.svg', { cache: 'no-store' });
            const svgText = await res.text();
            host.innerHTML = svgText;
            const svg = host.querySelector('svg');
            if (!svg) return null;
            svg.setAttribute('id', 'atlas-bd-map-svg');
            svg.setAttribute('class', 'w-full h-full max-h-[620px]');
            svg.setAttribute('preserveAspectRatio', 'xMidYMid meet');
            svg.querySelectorAll('path[id]').forEach(path => {
                const rawName = path.getAttribute('id') || '';
                const bbox = path.getBBox();
                const viewBox = svg.viewBox && svg.viewBox.baseVal ? svg.viewBox.baseVal : { width: 800, height: 1034 };
                ATLAS_SVG_DOTS[canonicalAtlasDistrictName(rawName)] = {
                    x: Number((((bbox.x + (bbox.width / 2)) / (viewBox.width || 800)) * 100).toFixed(2)),
                    y: Number((((bbox.y + (bbox.height / 2)) / (viewBox.height || 1034)) * 100).toFixed(2)),
                };
                path.style.cursor = 'pointer';
                path.style.fill = '#111111';
                path.style.stroke = '#5B544B';
                path.style.strokeWidth = '1';
                path.style.transition = 'fill 150ms ease, opacity 150ms ease';
                path.addEventListener('mouseenter', () => {
                    if (!path.classList.contains('atlas-svg-active')) path.style.fill = '#2D2722';
                });
                path.addEventListener('mouseleave', () => {
                    if (!path.classList.contains('atlas-svg-active')) path.style.fill = '#111111';
                });
                path.addEventListener('click', () => {
                    const district = getAtlasDistrictRecordByName(rawName);
                    if (!district) {
                        showToast(`No district record found for ${rawName}.`, 'error');
                        return;
                    }
                    const mapPoint = getAtlasMapPointByName(rawName) || {};
                    const dot = ATLAS_SVG_DOTS[canonicalAtlasDistrictName(rawName)] || mapPoint.dot || { x: 0, y: 0 };
                    const shopLink = mapPoint.shop_link || `/shop?district=${encodeURIComponent(district.name)}`;
                    ATLAS_PENDING_SELECTION = { id: district.id, name: district.name, dot, shop_link: shopLink };
                    setAtlasMapDot(dot.x, dot.y);
                    const nameEl = document.getElementById('atlas-map-selected-name');
                    const metaEl = document.getElementById('atlas-map-selected-meta');
                    if (nameEl) nameEl.textContent = district.name;
                    if (metaEl) metaEl.textContent = `Map dot ${Number(dot.x).toFixed(2)}%, ${Number(dot.y).toFixed(2)}% • ${shopLink}`;
                    updateAtlasSvgActiveState(district.name);
                    renderAtlasMapPickerList();
                });
            });
            ATLAS_SVG_READY = true;
            return svg;
        }
        ...
        function hydrateFromPayload(payload) {
            ...
            FEATURED_QUEUE = data.featured_queue || FEATURED_QUEUE;
            ...
            flashStartDate = (data.artisan_hour && data.artisan_hour.start_date) || flashStartDate || new Date().toISOString().split('T')[0];
            flashEndDate = (data.artisan_hour && data.artisan_hour.end_date) || flashEndDate || new Date().toISOString().split('T')[0];
        ...
        async function adminApi(url, options = {}) {
            ...
            if (!res.ok || data.ok === false) throw new Error(data.error || data.message || raw || 'request_failed');
            return data;
        }
        ...
        function renderMainContent() {
            ...
            const renderers = { 
                ...
                'heritage-atlas': renderHeritageAtlas
            };
        ...
        async function submitModalAction() {
            ...
            let endpoint = null;
            if(currentView === 'verification' || currentView === 'seller-details') endpoint = `/api/admin/verification/${currentModalId}/action`;
            else if(currentView === 'qc' || currentView === 'qc-details') endpoint = `/api/admin/qc/${currentModalId}/action`;
            else if(currentView === 'gi-verify' || currentView === 'gi-details' || currentView === 'flash-sale') endpoint = `/api/admin/gi/${currentModalId}/action`;
            if(!endpoint) return showToast('Unsupported action context.', 'error');
            try {
                await adminApi(endpoint, { method: 'POST', body: JSON.stringify({ action: currentModalAction, note }) });
                closeNoteModal();
                await refreshAdminData();
                if(currentView === 'seller-details') currentView = 'verification';
                if(currentView === 'qc-details') currentView = 'qc';
                if(currentView === 'gi-details') currentView = 'gi-verify';
                renderMainContent();
            ...
            return `
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 fade-in">
                    ...
                    <div class="bg-white p-8 rounded-3xl border border-[#E8E3DB] shadow-[0_4px_24px_-8px_rgba(0,0,0,0.02)] flex flex-col justify-between hover-lift min-h-[220px] group">
                        ...
                        <button onclick="openDocumentViewer('verification', ${v.id})" class="w-full py-2.5 mb-4 bg-[#FAF8F5] border border-[#E8E3DB] text-[#1A1614] rounded-xl text-[12px] font-medium hover:bg-white hover:border-[#C27803]/30 transition-all flex items-center justify-center gap-2"><i class="ph-bold ph-eye"></i> View Documents</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 fade-in">
                    ${MOCK_QC.map(q => `
                        <div class="bg-white p-7 rounded-3xl border border-[#E8E3DB] premium-shadow flex flex-col hover-lift group">
                            ...
                            <button onclick="openDocumentViewer('qc', ${q.id})" class="w-full py-2.5 mb-4 bg-[#FAF8F5] border border-[#E8E3DB] text-[#1A1614] rounded-xl text-[12px] font-medium hover:bg-white hover:border-[#C27803]/30 transition-all flex items-center justify-center gap-2"><i class="ph-bold ph-magnifying-glass"></i> View QC Docs</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 fade-in">
                    ${MOCK_GI_REQUESTS.map(req => `
                        <div class="bg-white p-8 rounded-3xl border border-[#E8E3DB] premium-shadow flex flex-col hover-lift group">
                            ...
                            <button onclick="openDocumentViewer('gi', ${req.id})" class="w-full py-2.5 mb-4 bg-white border border-[#E8E3DB] text-[#1A1614] rounded-xl text-[12px] font-medium hover:bg-[#FAF8F5] hover:border-[#C27803]/30 transition-all flex items-center justify-center gap-2 shadow-sm"><i class="ph-bold ph-books"></i> View Dossier</button>
                            <div class="mt-auto grid grid-cols-3 gap-2">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 fade-in">
                    ...
                    <div class="lg:col-span-4 bg-[#1A1614] rounded-3xl p-8 text-white relative overflow-hidden shadow-[0_12px_32px_-12px_rgba(26,22,20,0.5)] flex flex-col justify-between border border-[#C27803]/20">
                        ...
                        <div class="relative z-10 space-y-6">
                            ...
                            <input type="date" id="flash-start-date" value="${flashStartDate || new Date().toISOString().split('T')[0]}" class="hidden"><input type="date" id="flash-end-date" value="${flashEndDate || flashStartDate || new Date().toISOString().split('T')[0]}" class="hidden">
            ...
            return `
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 fade-in">
                    ...
                    <div class="bg-white p-8 rounded-3xl border border-[#E8E3DB] shadow-[0_4px_24px_-8px_rgba(0,0,0,0.02)] flex flex-col h-[500px]">
                        <div class="mb-6 pb-6 border-b border-[#FAF8F5]"><h3 class="font-serif text-xl text-[#1A1614] mb-1">Selection Queue</h3><p class="text-[12px] text-[#A39A8E]">Top 5 sellers from yesterday's sales appear here daily.</p></div>
                        <div class="flex-1 overflow-y-auto space-y-2 pr-2">
                            ${(FEATURED_QUEUE || []).map(s => `
                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-white border border-[#E8E3DB] hover:border-[#C27803]/40 cursor-pointer group transition-all shadow-sm hover:shadow-md">
                                    <img src="${s.img}" class="w-12 h-12 rounded-full bg-[#FAF8F5] border border-[#E8E3DB] object-cover">
                                    <div class="flex-1">
                                        <h4 class="font-medium text-[13px] text-[#1A1614] group-hover:text-[#C27803] transition-colors mb-0.5">${s.name}</h4>
                                        <div class="flex items-center gap-1.5 text-[11px] font-medium text-[#A39A8E] flex-wrap"><span class="text-[#C27803]"><i class="ph-fill ph-star"></i> ${s.rating}</span><span>• ${s.district}</span></div>
                                        <div class="text-[10px] text-[#6B635A] mt-1">Yesterday: ${s.yesterday_sales_fmt || 'BDT 0'} • ${s.items_sold || 0} sold</div>
                                    </div>
        ...
        function renderHeritageAtlas() {
            const rows = (MOCK_ATLAS_DATA || []).length ? MOCK_ATLAS_DATA : [];
            return `
                ${renderHeader('Heritage Atlas', 'Manage district-wise interactive map data and geographical features.', `<div class="flex items-center gap-3"><button onclick="openAtlasCreateModal()" class="bg-[#1A1614] text-white px-5 py-2.5 rounded-full text-[13px] font-medium hover:bg-[#C27803] transition-colors flex items-center gap-2 hover-lift"><i class="ph-bold ph-plus"></i> Add New Entry</button></div>`) }
                <div class="bg-white rounded-3xl border border-[#E8E3DB] shadow-[0_4px_24px_-8px_rgba(0,0,0,0.02)] overflow-hidden fade-in">
                    <div class="p-5 border-b border-[#FAF8F5] flex items-center justify-between gap-4">
                        <div>
                            <h3 class="font-serif text-2xl text-[#1A1614]">64 Districts Mapping Data</h3>
                            <p class="text-[12px] text-[#A39A8E] mt-1">Pick a zilla from the map, then fill density, product, category, story, and image. Shop link and map dot are auto-generated.</p>
                        </div>
                        <div class="text-[11px] font-medium text-[#A39A8E] bg-[#FAF8F5] px-3 py-2 rounded-xl border border-[#E8E3DB]">${rows.length} Entries</div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[1080px]">
                            <thead class="text-[10px] font-bold text-[#A39A8E] uppercase tracking-widest border-b border-[#FAF8F5]">
                                <tr><th class="px-6 py-4 font-normal">District Name</th><th class="px-6 py-4 font-normal">Featured Product</th><th class="px-6 py-4 font-normal">Category</th><th class="px-6 py-4 text-center font-normal">Density</th><th class="px-6 py-4 text-center font-normal">Map Dot</th><th class="px-6 py-4 text-center font-normal">Visibility</th><th class="px-6 py-4 text-right font-normal">Actions</th></tr>
                            </thead>
                            <tbody class="divide-y divide-[#FAF8F5]">
                                ${rows.map(d => `
                                    <tr class="hover:bg-[#FAF8F5]/50 transition-colors group">
                                        <td class="px-6 py-4"><div class="font-medium text-[13px] text-[#1A1614] group-hover:text-[#C27803] transition-colors">${d.district}</div><div class="text-[10px] font-mono text-[#A39A8E] mt-0.5">Coords: ${Number(d.x || 0).toFixed(2)}%, ${Number(d.y || 0).toFixed(2)}%</div></td>
                                        <td class="px-6 py-4"><span class="text-[13px] font-medium text-[#6B635A]">${d.product || '<span class="text-[#A39A8E] italic">Unassigned</span>'}</span></td>
                                        <td class="px-6 py-4"><span class="px-2.5 py-1 bg-[#FAF8F5] text-[#6B635A] rounded-lg capitalize text-[11px] font-medium border border-[#E8E3DB]">${d.category || 'none'}</span></td>
                                        <td class="px-6 py-4 text-center"><span class="px-2.5 py-1 rounded-lg capitalize text-[10px] font-bold tracking-wide border ${d.density === 'high' ? 'bg-red-50 text-red-600 border-red-100' : d.density === 'med' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-500 border-slate-100'}">${d.density || 'low'}</span></td>
                                        <td class="px-6 py-4 text-center"><span class="inline-flex items-center gap-2 text-[11px] text-[#6B635A]"><span class="w-2.5 h-2.5 rounded-full bg-[#C27803]"></span>${Number(d.x || 0).toFixed(1)}, ${Number(d.y || 0).toFixed(1)}</span></td>
                                        <td class="px-6 py-4 text-center"><button onclick="toggleAtlasStatus(${d.id})" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${d.isActive ? 'bg-[#C27803]' : 'bg-[#E8E3DB]'}"><span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${d.isActive ? 'translate-x-6' : 'translate-x-1'} shadow-sm"></span></button></td>
                                        <td class="px-6 py-4 text-right"><div class="flex items-center justify-end gap-2"><button onclick="openAtlasEditModal(${d.id})" class="px-4 py-2 border border-[#E8E3DB] bg-white rounded-lg hover:bg-[#1A1614] hover:text-white hover:border-[#1A1614] transition-all text-[12px] font-medium shadow-sm flex items-center gap-1.5"><i class="ph-bold ph-pencil-simple"></i> Edit</button><button onclick="deleteAtlasEntry(${d.id})" class="px-4 py-2 border border-red-200 text-red-600 bg-white rounded-lg hover:bg-red-50 transition-all text-[12px] font-medium shadow-sm flex items-center gap-1.5"><i class="ph-bold ph-trash"></i> Delete</button></div></td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
        ...
        async function loadAtlasDistrictOptions(force = false) {
            if (ATLAS_DISTRICTS.length && !force) return ATLAS_DISTRICTS;
            const [resp, mapResp] = await Promise.all([
                adminApi('/api/admin/atlas/district-options'),
                fetch('/static/assets/data/districts.json', { cache: 'no-store' }).then(r => r.ok ? r.json() : [])
            ]);
            ATLAS_DISTRICTS = resp.districts || [];
            ATLAS_MAP_POINTS = Array.isArray(mapResp) ? mapResp : [];
            return ATLAS_DISTRICTS;
        }

        function findAtlasMapPointByDistrictName(name) {
            return getAtlasMapPointByName(name);
        }

        function applyAtlasDistrictSelection(districtId, districtName, coords = {}, shopLink = '') {
            const hidden = document.getElementById('atlas-district');
            const selected = document.getElementById('atlas-selected-district');
            const linkInput = document.getElementById('atlas-link');
            const xInput = document.getElementById('atlas-x');
            const yInput = document.getElementById('atlas-y');
            if (hidden) hidden.value = districtId ? String(districtId) : '';
            if (selected) selected.textContent = districtName || 'No district selected';
            if (linkInput) linkInput.value = shopLink || (districtName ? `/shop?district=${encodeURIComponent(districtName)}` : '');
            if (xInput) xInput.value = coords && coords.x != null ? Number(coords.x).toFixed(2) : '';
            if (yInput) yInput.value = coords && coords.y != null ? Number(coords.y).toFixed(2) : '';
            if (coords && coords.x != null && coords.y != null) setAtlasMapDot(coords.x, coords.y);
            updateAtlasSvgActiveState(districtName || '');
        }

        function setAtlasMapDot(x, y) {
            const dot = document.getElementById('atlas-map-dot');
            const coords = document.getElementById('atlas-map-coords');
            if (!dot) return;
            dot.classList.remove('hidden');
            dot.style.left = `${x}%`;
            dot.style.top = `${y}%`;
            if (coords) coords.textContent = `Selected coordinates: ${Number(x).toFixed(2)}%, ${Number(y).toFixed(2)}%`;
        }

        function shouldHideDistrictInAtlasPicker(district) {
            if (!district) return true;
            const hasExistingEntry = district.atlas_id != null && district.atlas_id !== '';
            if (!currentModalId) return hasExistingEntry;
            return false;
        }

        function renderAtlasMapPickerList() {
            const list = document.getElementById('atlas-map-list');
            const markers = document.getElementById('atlas-map-markers');
            const search = (document.getElementById('atlas-map-search')?.value || '').trim().toLowerCase();
            if (!list || !markers) return;
            const rows = (ATLAS_DISTRICTS || []).map(d => {
                const mapPoint = findAtlasMapPointByDistrictName(d.name) || {};
                return { ...d, dot: mapPoint.dot || null, shop_link: mapPoint.shop_link || `/shop?district=${encodeURIComponent(d.name)}` };
            }).filter(d => d.dot && !shouldHideDistrictInAtlasPicker(d) && (!search || d.name.toLowerCase().includes(search)));
            list.innerHTML = rows.map(d => `
                <button type="button" onclick="selectAtlasMapDistrict(${d.id})" class="w-full text-left p-3 rounded-2xl border ${ATLAS_PENDING_SELECTION && Number(ATLAS_PENDING_SELECTION.id)===Number(d.id) ? 'bg-amber-50 border-[#C27803]/30' : 'bg-[#FAF8F5] border-[#E8E3DB] hover:bg-white'} transition-all">
                    <div class="font-medium text-[13px] text-[#1A1614]">${d.name}</div>
                    <div class="text-[11px] text-[#A39A8E] mt-1">Dot ${Number(d.dot.x).toFixed(1)}%, ${Number(d.dot.y).toFixed(1)}%</div>
                </button>`).join('');
            const activeDot = ATLAS_PENDING_SELECTION && ATLAS_PENDING_SELECTION.dot ? ATLAS_PENDING_SELECTION.dot : null;
            markers.innerHTML = activeDot ? `
                <div class="absolute -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none" style="left:${activeDot.x}%; top:${activeDot.y}%">
                    <span class="w-4 h-4 rounded-full bg-[#C27803] ring-4 ring-[#C27803]/20 block shadow-lg shadow-[#C27803]/30"></span>
                </div>` : '';
        }

        window.selectAtlasMapDistrict = (districtId) => {
            const district = (ATLAS_DISTRICTS || []).find(d => Number(d.id) === Number(districtId));
            if (!district) return;
            if (shouldHideDistrictInAtlasPicker(district)) return showToast('This district already has an entry. Use Edit for existing data.', 'error');
            const mapPoint = findAtlasMapPointByDistrictName(district.name);
            if (!mapPoint || !mapPoint.dot) return showToast('Map coordinates were not found for this district.', 'error');
            ATLAS_PENDING_SELECTION = {
                id: district.id,
                name: district.name,
                dot: mapPoint.dot,
                shop_link: mapPoint.shop_link || `/shop?district=${encodeURIComponent(district.name)}`,
            };
            setAtlasMapDot(mapPoint.dot.x, mapPoint.dot.y);
            const nameEl = document.getElementById('atlas-map-selected-name');
            const metaEl = document.getElementById('atlas-map-selected-meta');
            if (nameEl) nameEl.textContent = district.name;
            if (metaEl) metaEl.textContent = `Map dot ${Number(mapPoint.dot.x).toFixed(2)}%, ${Number(mapPoint.dot.y).toFixed(2)}% • ${ATLAS_PENDING_SELECTION.shop_link}`;
            updateAtlasSvgActiveState(district.name);
            renderAtlasMapPickerList();
        };

        window.applyAtlasMapSelection = () => {
            if (!ATLAS_PENDING_SELECTION) return showToast('Please select a zilla from the map.', 'error');
            applyAtlasDistrictSelection(ATLAS_PENDING_SELECTION.id, ATLAS_PENDING_SELECTION.name, ATLAS_PENDING_SELECTION.dot, ATLAS_PENDING_SELECTION.shop_link);
            closeAtlasMapPicker();
        };

        window.openAtlasCreateModal = async () => {
            currentModalId = null;
            await loadAtlasDistrictOptions(true);
            ATLAS_PENDING_SELECTION = null;
            document.getElementById('atlas-modal-title').textContent = 'New Heritage Atlas Entry';
            document.getElementById('atlas-save-btn').textContent = 'Create New Entry';
            document.getElementById('atlas-delete-btn').classList.add('hidden');
            document.getElementById('atlas-density').value = 'low';
            document.getElementById('atlas-category').value = 'other';
            document.getElementById('atlas-product').value = '';
            document.getElementById('atlas-story').value = '';
            document.getElementById('atlas-img').value = '';
            applyAtlasDistrictSelection('', '', {}, '');
            document.getElementById('atlas-map-dot').classList.add('hidden');
            document.getElementById('atlas-map-coords').textContent = 'Map theke district e direct click korun, ba right side list theke select korun. New entry only unused district er jonno.';
            document.getElementById('atlas-edit-modal').classList.remove('hidden');
        };

        window.openAtlasEditModal = async (id) => {
            const data = (MOCK_ATLAS_DATA || []).find(x => Number(x.id) === Number(id));
            if (!data) return showToast('Atlas entry not found.', 'error');
            currentModalId = id;
            await loadAtlasDistrictOptions(true);
            const district = (ATLAS_DISTRICTS || []).find(d => Number(d.id) === Number(data.district_id));
            const mapPoint = district ? findAtlasMapPointByDistrictName(district.name) : null;
            ATLAS_PENDING_SELECTION = district ? {
                id: district.id,
                name: district.name,
                dot: { x: Number(data.x || (mapPoint?.dot?.x || 0)), y: Number(data.y || (mapPoint?.dot?.y || 0)) },
                shop_link: data.link || mapPoint?.shop_link || `/shop?district=${encodeURIComponent(district.name)}`,
            } : null;
            document.getElementById('atlas-modal-title').textContent = `Edit Atlas: ${data.district}`;
            document.getElementById('atlas-save-btn').textContent = 'Save Entry';
            document.getElementById('atlas-delete-btn').classList.remove('hidden');
            document.getElementById('atlas-density').value = data.density || 'low';
            document.getElementById('atlas-category').value = data.category || 'other';
            document.getElementById('atlas-product').value = data.product || '';
            document.getElementById('atlas-story').value = data.story || '';
            document.getElementById('atlas-img').value = data.img || '';
            applyAtlasDistrictSelection(data.district_id, data.district, { x: data.x || mapPoint?.dot?.x || 0, y: data.y || mapPoint?.dot?.y || 0 }, data.link || mapPoint?.shop_link || '');
            document.getElementById('atlas-edit-modal').classList.remove('hidden');
        };
        window.closeAtlasEditModal = () => document.getElementById('atlas-edit-modal').classList.add('hidden');
        window.openAtlasMapPicker = async () => {
            await loadAtlasDistrictOptions(false);
            document.getElementById('atlas-map-modal').classList.remove('hidden');
            await ensureAtlasSvgLoaded();
            renderAtlasMapPickerList();
            if (ATLAS_PENDING_SELECTION?.dot) {
                setAtlasMapDot(ATLAS_PENDING_SELECTION.dot.x, ATLAS_PENDING_SELECTION.dot.y);
                const nameEl = document.getElementById('atlas-map-selected-name');
                const metaEl = document.getElementById('atlas-map-selected-meta');
                if (nameEl) nameEl.textContent = ATLAS_PENDING_SELECTION.name;
                if (metaEl) metaEl.textContent = `Map dot ${Number(ATLAS_PENDING_SELECTION.dot.x).toFixed(2)}%, ${Number(ATLAS_PENDING_SELECTION.dot.y).toFixed(2)}% • ${ATLAS_PENDING_SELECTION.shop_link}`;
                updateAtlasSvgActiveState(ATLAS_PENDING_SELECTION.name);
            }
        };
        window.closeAtlasMapPicker = () => document.getElementById('atlas-map-modal').classList.add('hidden');
        window.saveAtlasEdit = async () => {
            const payload = {
                district_id: document.getElementById('atlas-district').value,
                density: document.getElementById('atlas-density').value,
                category: document.getElementById('atlas-category').value,
                product: document.getElementById('atlas-product').value,
                story: document.getElementById('atlas-story').value,
                img: document.getElementById('atlas-img').value,
                link: document.getElementById('atlas-link').value,
                x: document.getElementById('atlas-x').value,
                y: document.getElementById('atlas-y').value,
            };
            if (!payload.district_id) return showToast('Please pick a zilla from the map.', 'error');
            const endpoint = currentModalId ? `/api/admin/atlas/${currentModalId}` : '/api/admin/atlas';
            const message = currentModalId ? 'Atlas entry updated.' : 'Atlas entry created.';
            await withRefresh(message, () => adminApi(endpoint, { method: 'POST', body: JSON.stringify(payload) }));
            closeAtlasEditModal();
            setActiveView('heritage-atlas');
        };
        window.deleteAtlasEntry = async (id = null) => {
            const targetId = id || currentModalId;
            if (!targetId) return;
            if (!confirm('Delete this heritage atlas entry?')) return;
            await withRefresh('Atlas entry deleted.', () => adminApi(`/api/admin/atlas/${targetId}`, { method: 'DELETE' }));
            closeAtlasEditModal();
            setActiveView('heritage-atlas');
        };
        window.toggleAtlasStatus = async (id) => {
            await withRefresh('Atlas visibility updated.', () => adminApi(`/api/admin/atlas/${id}/toggle`, { method: 'POST' }));
            setActiveView('heritage-atlas');
        };
                            ...
                            <div class="flex-1 p-10 md:border-r border-[#E8E3DB] relative overflow-hidden group">
                                ...
                                <div class="space-y-6 relative z-10">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-[#C27803] mt-2 font-mono">Email is loaded from the database and reflects the current admin account.</p>
                                    </div>
        ...
        function renderDocumentViewerList() {
            const docs = DOC_VIEWER_STATE.docs || [];
            const listEl = document.getElementById('doc-list');
            const search = (document.getElementById('doc-search')?.value || '').trim().toLowerCase();
            if (!listEl) return;
            const filtered = docs.filter(d => !search || `${d.label} ${d.meta}`.toLowerCase().includes(search));
            listEl.innerHTML = filtered.length ? filtered.map(doc => `
                <button data-id="${doc.id}" class="doc-item w-full text-left p-4 rounded-2xl border transition-all ${doc.id === DOC_VIEWER_STATE.activeId ? 'bg-white border-[#C27803]/40 shadow-sm' : 'bg-[#FAF8F5] border-[#E8E3DB] hover:bg-white'}">
                    <div class="flex items-start gap-3">
                        <div class="mt-0.5 w-10 h-10 rounded-2xl flex items-center justify-center ${doc.kind === 'image' ? 'bg-amber-50 text-[#C27803]' : doc.kind === 'pdf' ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-600'}"><span class="text-[11px] font-bold uppercase">${doc.kind}</span></div>
                        <div class="min-w-0 flex-1"><div class="text-[13px] font-semibold text-[#1A1614] truncate">${doc.label}</div><div class="text-[11px] text-[#A39A8E] mt-1 line-clamp-2">${doc.meta || 'No extra metadata'}</div></div>
                    </div>
                </button>`).join('') : `<div class="p-6 text-[13px] text-[#A39A8E]">No matching documents.</div>`;
            listEl.querySelectorAll('.doc-item').forEach(btn => btn.addEventListener('click', () => setActiveDocument(Number(btn.dataset.id))));
        }

        function renderDocumentViewerStage(doc) {
            ...
            if (!stageEl || !titleEl || !metaEl || !controlsEl) return;
            if (!doc) {
            ...
            titleEl.textContent = doc.label || 'Document';
            metaEl.textContent = doc.meta || '';
            if (doc.kind === 'image' && doc.url && doc.url !== '#') {
                controlsEl.classList.remove('hidden'); controlsEl.classList.add('flex');
                stageEl.innerHTML = `<div class="min-h-full flex items-center justify-center"><img id="stage-image" src="${doc.url}" alt="${escapeHtml(doc.label)}" style="transform: scale(${DOC_VIEWER_STATE.zoom}); transform-origin: center center; max-width:100%; height:auto; transition:transform .15s ease;" class="rounded-3xl border border-[#E8E3DB] bg-white shadow-sm"></div>`;
                return;
            }
            controlsEl.classList.add('hidden'); controlsEl.classList.remove('flex'); DOC_VIEWER_STATE.zoom = 1;
            if ((doc.kind === 'pdf' || doc.kind === 'file') && doc.url && doc.url !== '#') {
                ...
                stageEl.innerHTML = `<div class="min-h-[65vh] flex items-center justify-center">
                    <div class="max-w-2xl w-full bg-white border border-[#E8E3DB] rounded-[28px] p-8 shadow-sm">
                        ...
                        <h3 class="font-serif text-3xl text-[#1A1614]">${escapeHtml(doc.label || 'Document')}</h3>
                        ...
                        ${doc.meta ? `<div class="mt-4 text-[12px] text-[#8B7F72]">${escapeHtml(doc.meta)}</div>` : ''}
            ...
            if (doc.kind === 'text') {
                stageEl.innerHTML = `<pre class="whitespace-pre-wrap break-words text-[13px] leading-6 bg-white border border-[#E8E3DB] rounded-3xl p-6 min-h-[65vh]">${escapeHtml(doc.text_content || 'No text preview available.')}</pre>`;
                return;
            }
        ...
        function setActiveDocument(id) {
            DOC_VIEWER_STATE.activeId = id;
            renderDocumentViewerList();
            renderDocumentViewerStage((DOC_VIEWER_STATE.docs || []).find(d => Number(d.id) === Number(id)));
        }
        ...
        async function openDocumentViewer(kind, id) {
            const modal = document.getElementById('document-viewer-modal');
            const viewerTitle = document.getElementById('viewer-title');
            const stageEl = document.getElementById('doc-stage');
            if (!modal || !viewerTitle || !stageEl) return;
            modal.classList.remove('hidden');
            viewerTitle.textContent = 'Loading…';
            stageEl.innerHTML = `<div class="h-full flex items-center justify-center text-[#A39A8E]">Loading documents…</div>`;
            DOC_VIEWER_STATE = { docs: [], activeId: null, zoom: 1 };
            try {
                const res = await fetch(`/api/admin/documents/${kind}/${id}`, { credentials: 'same-origin' });
                const raw = await res.text();
                let data = {};
                try { data = raw ? JSON.parse(raw) : {}; } catch (_) {}
                if (!res.ok || data.ok === false) throw new Error(data.error || data.message || raw || 'viewer_load_failed');
                viewerTitle.textContent = data.title || 'Document Viewer';
                DOC_VIEWER_STATE.docs = data.documents || [];
                renderDocumentViewerList();
                if (DOC_VIEWER_STATE.docs.length) setActiveDocument(DOC_VIEWER_STATE.docs[0].id);
                else renderDocumentViewerStage(null);
            } catch (e) {
                viewerTitle.textContent = 'Document Viewer';
                stageEl.innerHTML = `<div class="h-full flex items-center justify-center text-red-600">${escapeHtml(e.message || 'Failed to load documents.')}</div>`;
            }
        }
        ...
        document.addEventListener('input', (e) => { if (e.target && e.target.id === 'atlas-map-search') renderAtlasMapPickerList(); if (e.target && e.target.id === 'doc-search') renderDocumentViewerList(); });

        document.addEventListener('click', (e) => { const btn = e.target.closest('[data-zoom]'); if (!btn) return; const action = btn.dataset.zoom; if (action === 'in') DOC_VIEWER_STATE.zoom = Math.min(3, DOC_VIEWER_STATE.zoom + 0.2); if (action === 'out') DOC_VIEWER_STATE.zoom = Math.max(0.4, DOC_VIEWER_STATE.zoom - 0.2); if (action === 'reset') DOC_VIEWER_STATE.zoom = 1; renderDocumentViewerStage((DOC_VIEWER_STATE.docs || []).find(d => Number(d.id) === Number(DOC_VIEWER_STATE.activeId))); });
        ...
        window.selectTodaysArtisan = (id) => withRefresh('Featured artisan updated.', () => adminApi(`/api/admin/featured-artisan/${id}`, { method: 'POST' }));
        window.publishSiteUpdates = async () => {
            heroHeadlineText = (document.getElementById('site-hero-headline') || {}).value || heroHeadlineText;
            heroSubtext = (document.getElementById('site-hero-subtext') || {}).value || heroSubtext;
            heroImageUrl = (document.getElementById('site-hero-image-url') || {}).value || heroImageUrl;
        ...
        document.addEventListener('click', (event) => {
            const map = document.getElementById('atlas-map-canvas');
            if (!map || !map.contains(event.target) || event.target.closest('button')) return;
            const rect = map.getBoundingClientRect();
            const x = Math.max(0, Math.min(100, ((event.clientX - rect.left) / rect.width) * 100));
            const y = Math.max(0, Math.min(100, ((event.clientY - rect.top) / rect.height) * 100));
            const candidates = (ATLAS_DISTRICTS || []).map(d => {
                const mapPoint = findAtlasMapPointByDistrictName(d.name);
                if (!mapPoint || !mapPoint.dot) return null;
                const dx = Number(mapPoint.dot.x) - x;
                const dy = Number(mapPoint.dot.y) - y;
                return { district: d, mapPoint, distance: Math.sqrt(dx * dx + dy * dy) };
            }).filter(Boolean).sort((a, b) => a.distance - b.distance);

            if (candidates.length) {
                const chosen = candidates[0];
                selectAtlasMapDistrict(chosen.district.id);
            } else {
                document.getElementById('atlas-x').value = x.toFixed(2);
                document.getElementById('atlas-y').value = y.toFixed(2);
                setAtlasMapDot(x, y);
            }
        });
