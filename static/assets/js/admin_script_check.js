        ...
        async function openDocumentViewer(kind, id) {
            const endpointMap = {
                verification: `/api/admin/verification/${id}/documents`,
                qc: `/api/admin/qc/${id}/documents`,
                gi: `/api/admin/gi/${id}/documents`
            };
            const endpoint = endpointMap[kind];
            if (!endpoint) return showToast('Viewer unavailable.', 'error');
            try {
                const resp = await adminApi(endpoint);
                const docs = resp.documents || [];
                const title = resp.title || 'Document Viewer';
                const rows = docs.map(d => {
                    const label = d.label || 'Document';
                    const meta = d.uploaded_at || '';
                    if (d.url && d.url !== '#') {
                        return `<li class="mb-3"><a href="${d.url}" target="_blank" class="text-[#C27803] font-medium hover:underline">${label}</a><div class="text-[11px] text-[#6B635A] mt-1">${meta}</div></li>`;
                    }
                    return `<li class="mb-3"><div class="font-medium text-[#1A1614]">${label}</div><div class="text-[11px] text-[#6B635A] mt-1">${meta}</div></li>`;
                }).join('');
                const win = window.open('', '_blank', 'width=760,height=680');
                if (!win) return showToast('Please allow pop-ups for document view.', 'error');
                win.document.write(`<!doctype html><html><head><title>${title}</title><style>body{font-family:Inter,Arial,sans-serif;padding:24px;background:#faf8f5;color:#1A1614} .card{max-width:760px;margin:0 auto;background:#fff;border:1px solid #E8E3DB;border-radius:24px;padding:24px} h1{font-family:Georgia,serif;margin:0 0 8px} ul{padding-left:18px} a{color:#C27803;text-decoration:none} a:hover{text-decoration:underline}</style></head><body><div class="card"><h1>${title}</h1><p>Available files and references for review.</p><ul>${rows || '<li>No documents found.</li>'}</ul></div></body></html>`);
                win.document.close();
            } catch (e) {
                showToast(e.message || 'Document viewer failed', 'error');
            }
        }
